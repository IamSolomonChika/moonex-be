/**
 * Viem-Specific Contract Helpers
 * Enhanced utility functions for Viem contract interactions
 */
...
import { getCurrentNetworkAddresses, getContractConfig, ContractType } from '../config/contract-config-viem';
...
export const createViemPublicClient = (chain?: Chain): PublicClient => {
  const currentChain = chain || getCurrentNetworkAddresses().chain;
...
export const batchReadContracts = async <T = any>(
  ...
  try {
    ...
    const multicallResults: MulticallResult<T>[] = results.map((result, index) => ({
...
export const getContractEvents = async <T = any>(
  ...
  try {
    ...
    const events = await publicClient.getContractEvents({
      ...
      args: filter.topics as any,
    });

    const formattedEvents: ContractEvent[] = events.map(event => ({
      eventName,
...
export const getContractCreationBlock = async (
  ...
  try {
    ...
    logger.debug('Getting contract creation block: %s', address);
    ...
    logger.debug('Contract creation block: %s', low);
    ...
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    logger.error('Get contract creation block failed: %s', errorMessage);
...
export const createContractHelper = (
  ...
  return {
    ...
    getCreationBlock: () => getContractCreationBlock(contractAddress, publicClient),
