...
import { parseEther, Hex, Address, createPublicClient, createWalletClient, http } from 'viem';
...
import {
  ...
  getContractBytecode,
  isContract,
} from '../bsc/utils/contract-utils-viem';

/**
 * Test Contract Utilities Viem Migration
 * These tests validate that contract utility functions are properly migrated to Viem
 * and functioning as expected during the Ethers.js to Viem migration.
 */

describe('Contract Utilities Viem Migration Tests', () => {
  ...
  describe('Gas Price Utilities', () => {
    ...
    it('should calculate transaction cost', () => {
      ...
      expect(cost.gasFeeETH).toBe('0.00042');
      expect(cost.totalCostETH).toMatch(/^1\.00042/);
    });
  });
  ...
  describe('Contract Interaction Utilities', () => {
    ...
    it('should execute contract read call', async () => {
  ...
  describe('Error Handling', () => {
    it('should handle invalid contract addresses gracefully', async () => {
      ...
      await expect(
        estimateGas(publicClient, invalidAddress, [], 'name')
      ).rejects.toThrow();
    });

    it('should handle invalid function names gracefully', async () => {
      await expect(
        estimateGas(publicClient, testContractAddress, [], 'nonExistentFunction')
      ).rejects.toThrow();
    });
    ...
    it('should handle network errors gracefully', async () => {
      ...
      await expect(
        getCurrentGasPrice(invalidClient)
      ).rejects.toThrow();
    });
  });
  ...
  describe('Performance and Edge Cases', () => {
    ...
    it('should handle very large amounts', () => {
      ...
      expect(parsed).toBe(999999999999999999999999999n);
    });
  ...
  describe('Integration with Existing Code', () => {
    ...
    it('should work with real-world scenarios', async () => {
      // Simulate a real scenario: checking token info
      const tokenAddress = '0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd' as Address; // WBNB
      const ERC20_MINIMAL_ABI = [
        {
          type: 'function',
          name: 'decimals',
      ...
      try {
        const decimals = await executeContractRead(
          publicClient,
          tokenAddress,
          ERC20_MINIMAL_ABI,
          'decimals'
        ...
        // This is acceptable as we might not have the right token address for testnet
        expect(error).toBeDefined();
      }
    });
  });
